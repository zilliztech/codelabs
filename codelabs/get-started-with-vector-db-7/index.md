...
Hey there - welcome back to [Milvus tutorials](https://codelabs.milvus.io/). In the previous tutorial, we did a deep dive on two quantization strategies available in Milvus - scalar quantization and product quantization.

In this tutorial, we'll build on top of that knowledge by looking at what is perhaps the most commonly used primary algorithm today: Hierarchical Navigable Small Worlds (HNSW). HNSW performs very well when it comes to both speed and accuracy, making it an incredibly robust vector search algorithm. Despite it being popular, understanding HNSW can be a bit tricky. In this tutorial, we'll break down HNSW into its individual steps, developing our own simple implementation along the way.
...
<p style="text-align:center"><sub>HNSW, visualized.</sub></p>
...
As with the skip list, if a query vector appears in some layer `l`, if will appear in layer `l+1` with some pre-defined probability `p`, with `p` typically being set to `1/log2(L)`, where `L` is the total number of layers. Note how this does not actually constrain the minimum distance between any two vertices/vectors in a particular layer - it's entirely possible that we end up with a poorly constructed graph, but the probability that this happens is incredibly low.
...
Let's go back go the HNSW insert. We'll first need to figure out which layer to insert our new vector into:

```python
def _get_insert_layer(L, p):

    # recall that `p` is typically set to `1/log2(L)`
...
def insert(index, vec, L=5, efc=10):
    ...
    l = _get_insert_layer(L, 1/np.log2(L))
...
In this tutorial, we did a deep dive into HNSW, one of the most popular vector search strategies today. We also created a own implementation in Python. While we made significant use of numpy-based array math, Python is not the most ideal langugage for implementing HNSW due to interpreter overhead.
